...
if args.algorithm == "tr1":  # Algorithm 1
    ...
    trimmed_seq_lengths = []  # for downstream statistics
    trimmed_seq_qualities = []  # for downsteam statistics
    ...
    for record in records:  # Parse through each accession
        if len(record.seq) < 30:
            ...
            std_quality = numpy.std(
                record.letter_annotations["phred_quality"][0:50])
            threshold = mean_quality - (1 * std_quality)
            for i in range(0, (len(record.seq)-4)):  # Iterate over each base
                a = record.letter_annotations["phred_quality"][i]
                b = record.letter_annotations["phred_quality"][i+1]
                c = record.letter_annotations["phred_quality"][i+2]
                d = record.letter_annotations["phred_quality"][i+3]
                e = record.letter_annotations["phred_quality"][i+4]
                five_consecutive = [a, b, c, d, e]
                if all(q < threshold for q in five_consecutive):
                    # If condition is met
                    trimmed_sequences.append(record[0:i])  # up to i:th base
                    trimmed += 1
                    break  # stop the loop
                elif i == (len(record.seq)-5):  # If end of sequence
                    trimmed_sequences.append(record)  # keep whole record
                    not_trimmed += 1
